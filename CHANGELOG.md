...
### Changed
- **Feature access is now ban-based, not operator-locked** — the operator "feature lock" (global release flags that 403'd whole features for everyone) is replaced by per-player moderation gating. A shared backend `RequireNotBanned()` filter refuses only Banned/Restricted (banned/suspended) accounts on friends, party, messages, matchmaking, notifications, personalization, skills, experiments, ml and game-events; everyone else keeps access. `/app/config` and the client `FeatureFlags` now default every user feature **on** (realtime multiplayer, matchmaking, tournaments, skill tree, personalization, notifications, territory, guardians, experiments), so all routes are reachable. **Crypto stays gated** (flag + `Crypto:Enabled` + admin policy), real-money store purchases and dev-tooling stay off, and the separate parental-consent/guardian controls are untouched. See `docs/api/FEATURE_ACCESS_POLICY.md`

### Added
- **Champion vs Tier — sponsor-multiplier attribution (Phase 5)** — the jackpot multiplier that already existed on `GameEvent` was never set and anonymous; it is now real and attributable. Backend: `GameEvent.SponsorName` + `ApplySponsor(name, multiplier)` (clamps 1–10×, blank clears, no-op after close), a `GameEventSponsorAttribution` record (entity + migration `AddSponsorAttribution`) written once at close capturing base/effective jackpot, multiplier, the boost the sponsor funded, the rank-1 beneficiary and season — for reconciliation; admin `POST /admin/game-events/{id}/sponsor` (set) and `GET /admin/game-events/sponsor-attributions` (filtered report + per-sponsor totals); `SponsorName` surfaced on the status DTO. Client: `ChampionEvent.sponsorName` + `hasSponsor`, and a "Jackpot N× boosted by <sponsor>" attribution line on `ChampionVsTierCard`. Backend suite green (270 application tests incl. clamp/clear/idempotent-close coverage)
- **Champion vs Tier — premium spectator / elimination cam (Phase 4)** — basic live spectating stays **free** for everyone (rounds over SignalR, live counts, jackpot); a `spectator_premium` entitlement unlocks a running elimination-cam feed of who was just knocked out. Backend: `ChampionSpectatorService` (basic counts always; elimination feed gated on the pass), `GET /game-events/{id}/spectate` (anonymous → basic view) and admin `POST /admin/game-events/spectator-pass` to comp a permanent or seasonal pass. Client: `ChampionSpectatorView`/`ChampionElimination` models, `getSpectatorView` + `championSpectatorProvider`, and an elimination-cam panel on `ChampionLiveScreen` — premium shows the live feed (newest-first, champion-fell highlighted), free shows a locked upsell — refreshed on each round/duel resolve. 4 backend spectator tests
- **Champion vs Tier — no-loss prediction (Phase 3)** — the audience hook for the ~everyone who isn't one of the 100 participants. Spectators predict "will the champion defend the crown?" while the event is Open; correct predictors share a fixed coin pool (+ flat XP) at close, nothing is ever staked (safe for minor users). Backend: `ChampionPrediction` entity + migration `AddChampionPredictions`, `ChampionPredictionService` (predict/resolve/state), resolution wired into `EndMatchAsync`, `POST /game-events/{id}/predict` + `GET /game-events/{id}/prediction`, `ChampionPredictionOptions`-tunable pool/XP. Client: `ChampionPrediction` model, `getPrediction`/`submitPrediction`, and a `ChampionPredictionPanel` on the leaderboard card with Yes/No pick, a live crowd tally bar, and locked/result states. 5 backend + 3 client tests
- **Champion vs Tier — redundancy, duels & replay-on-join** — (1) a hosted `ChampionRoundWatchdog` runs alongside the Hangfire scheduler as a self-healing safety net: it sweeps for any overdue round/duel a dropped job missed and resolves it idempotently, so the two drivers never double-fire. (2) **Champion duels**: the champion can call out one alive challenger for a head-to-head on a single question (`POST /game-events/{id}/duel` + `/duel/answer`) — correct beats wrong then speed, champion takes ties, capped per match; only the two duelists are affected, the loser is culled and feeds the jackpot, a dethroned champion ends the match (`ChampionDuel` entity, migration `AddChampionDuels`, duel realtime contracts, Hangfire duel job). (3) **Replay-on-join**: `GET /game-events/{id}/live` returns the current open round/duel so `ChampionLiveScreen` renders the in-progress state on entry instead of waiting for the next broadcast. Client gains the duel/snapshot DTOs, `getLiveSnapshot`/`startChampionDuel`/`submitDuelAnswer`, and the full duel UI on `ChampionLiveScreen`: a champion "call out a challenger" roster picker (`GET /game-events/{id}/participants`) with a duels-remaining counter, a focused duelist answer view, and a spectator duel banner. 15 orchestration tests
...
### Changed
- **Token refresh over the secure channel (Phase 2, B-proactive)** — `/auth/refresh` now travels the KMS secure channel (satisfying the backend's `RequireSecureChannel`) instead of plain JSON, and is triggered **proactively** (`AuthSession.isExpiringSoon`) while the access token is still valid so the channel can be started with a valid bearer. To avoid refresh recursion (the normal `AuthHttpClient` auto-refreshes), the refresh path uses a dedicated `autoRefresh:false` transport + its own `EncryptedApiClient`, late-injected into `AuthApiClient`. **Requires a live KMS+backend smoke test before enabling** — see `docs/api/GUEST_IDENTITY_KMS_TIERING_PLAN.md`.
- **Guest gating for account-required surfaces** — added `isGuestProvider` / `hasUpgradedAccountProvider` (derived from the player identity) and a reusable `accountRequiredGuard` GoRouter guard. Anonymous device-guests hitting an account-required route (e.g. the crypto wallet, whose every action calls a secure-channel-gated endpoint a guest can't complete) are redirected to the account-upgrade prompt (`/account-link?from=…`) instead of a dead end. Nav entry points can hide the same surfaces via `isGuestProvider`.
...
- **Skill Tree back button crash** — tapping back on `/skills` hard-crashed: the screen called `Navigator.of(context).pop()`, but `/skills` is entered via `context.go()` (which replaces the stack), so popping the last page tripped GoRouter's `currentConfiguration.isNotEmpty` assertion and cascaded into a `Navigator.dispose` failure. Now uses the `context.safeBack()` helper (pop if possible, else fall back to `/home`).
- **Settings write clobbered by initial load** — `SettingsController` fired `_loadSettings()` unawaited, so an in-flight load could resolve `getPlayerName()` and overwrite a name the user had just set; the load future is now cached and awaited before mutating. Also cached `PlayerProfileService._getBox()`'s `openBox` future so concurrent callers can't double-open the settings box.
- **Skill tree visualization mobile overflows** — the analytics skill-tree header text, summary stat cards, and tier grid overflowed at phone widths (Flexible header, `Expanded` stat cards, taller grid `childAspectRatio`).
- **Full widget-test suite green** — cleared the remaining widget/assertion-drift failures (skill-tree, `SynaptixApp` smoke test, rendering-cache color equality, `removeFriend` route contract, premium-store reward-claim); `flutter test` now reports 7,418 pass / 11 skip / 0 fail with `flutter analyze` clean. See `docs/status/TEST_SUITE_TRIAGE.md` §4g–4h.
- **`tracked_tests` CI job** — `scripts/run_tests_with_tracking.sh`'s summary parser crashed (`'list' object has no attribute 'get'`) on a top-level JSON array line from `flutter test --machine`, failing the job even when every test passed; it now skips non-object JSON lines.
