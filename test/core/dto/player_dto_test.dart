...
import 'package:synaptix/core/dto/player_dto.dart';
...
void main() {
  ...
  Map<String, dynamic> fullPlayerJson() => {
        'id': 'p1',
        'username': 'alice',
  ...
  group('PlayerDto.fromJson', () {
    test('parses id', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).id, 'p1');
    });

    test('parses username', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).username, 'alice');
    });

    test('parses email', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).email, 'alice@example.com');
    });

    test('parses handle', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).handle, '@alice');
    });

    test('parses avatarUrl', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).avatarUrl,
          'https://example.com/avatar.png');
    });

    test('parses country', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).country, 'US');
    });

    test('parses ageGroup', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).ageGroup, 'adults');
    });

    test('parses role', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).role, 'user');
    });

    test('parses xp', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).xp, 1500);
    });

    test('parses level', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).level, 5);
    });
    ...
    test('country null when absent', () {
      final j = Map<String, dynamic>.from(fullPlayerJson())..remove('country');
    ...
    test('role defaults user when absent', () {
      final j = Map<String, dynamic>.from(fullPlayerJson())..remove('role');
      expect(PlayerDto.fromJson(j).role, 'user');
    });

    test('wallet deserialized as WalletDto', () {
      final p = PlayerDto.fromJson(fullPlayerJson());
    ...
    test('wallet from missing key defaults to empty wallet', () {
      final j = Map<String, dynamic>.from(fullPlayerJson())..remove('wallet');
  ...
  group('PlayerDto.toJson', () {
    test('contains id', () {
      expect(PlayerDto.fromJson(fullPlayerJson()).toJson()['id'], 'p1');
    });

    test('contains username', () {
      expect(
          PlayerDto.fromJson(fullPlayerJson()).toJson()['username'], 'alice');
    });

    test('wallet serialized as nested map', () {
      final j = PlayerDto.fromJson(fullPlayerJson()).toJson();
