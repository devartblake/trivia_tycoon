...
class _InviteScreenState extends ConsumerState<InviteScreen> {
    ...
    return Scaffold(
      ...
      body: referralAsync.when(
        data: (referralCode) => serviceAsync.when(
          ...
          error: (err, _) => _buildContentFallback(context, theme, referralCode),
        ),
  ...
  Widget _buildNavButton({required IconData icon, required VoidCallback onPressed, required ThemeData theme}) {
    return Container(
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              const Icon(Icons.card_giftcard_rounded, color: Color(0xFFFFD700), size: 40),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: const [
                    Text('Invite friends &', style: TextStyle(color: Colors.white, fontSize: 16)),
                    Text('get rewarded!', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildShareButton(String code, String link) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: const Text('SHARE INVITE', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Widget _buildInfoCard({required String title, required Widget child}) {
    return Column(
      ...
      children: [
        ...
        Card(
          elevation: 0,
          shape: RoundedRectangleBorder(side: BorderSide(color: Colors.grey[200]!), borderRadius: BorderRadius.circular(20)),
  ...
  Widget _buildLinkRow(BuildContext context, String link, ThemeData theme) {
    return Row(
      children: [
        Expanded(child: Text(link, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.grey[600]))),
        IconButton(icon: Icon(Icons.copy, size: 20, color: theme.primaryColor), onPressed: () => _copyToClipboard(context, link)),
      ],
    );
  }
  ...
  Widget _buildHowItWorks() {
    return Container(
      ...
      child: Column(
        children: [
          const Text('How it works', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
