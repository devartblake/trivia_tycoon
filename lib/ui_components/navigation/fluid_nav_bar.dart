...
class FluidNavBar extends StatefulWidget {
  static const double nominalHeight = 64.0;
  ...
  /// An optional builder to change or wrap the builder item
  ///
...
class _FluidNavBarState extends State<FluidNavBar> with TickerProviderStateMixin {
  ...
  void _handleScroll() {
    ...
    } else if (_scrollController.position.userScrollDirection == ScrollDirection.forward && !_isVisible) {
  ...
  void _handleTap(int index) {
    if (_currentIndex == index || _xController.isAnimating || _disposed) return;
    ...
    Future.delayed(
      ...
      () {
        if (_disposed) return; // Check disposed
  ...
  List<Widget> _buildButtons() {
    var buttons = widget.icons.asMap().entries.map((entry) {
...
class _BackgroundCurvePainter extends CustomPainter {
  ...
  void paint(canvas, size) {
    ...
    // Point linear to the top edge of the background pane
