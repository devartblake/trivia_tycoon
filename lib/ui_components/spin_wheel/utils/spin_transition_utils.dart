...
class SpinTransitionUtils {
  ...
  static double calculateTargetAngle({
    ...
    final targetAngle =
        fullCircle * spins + anglePerSegment * (totalSegments - targetIndex);
  ...
  static int getSegmentIndexFromAngle(double angle, int totalSegments) {
    ...
    return (totalSegments - (normalized / anglePerSegment).floor()) %
        totalSegments;
  }
  ...
  static double addOffsetWithinSegment(double angle, int totalSegments) {
    ...
    final random = Random();
    final offset =
        (random.nextDouble() - 0.5) * anglePerSegment * 0.4; // ±20% of segment
    return angle + offset;
  }
}
