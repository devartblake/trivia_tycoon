...
final analyticsManagerProvider =
    AsyncNotifierProvider<AnalyticsStreamManager, AnalyticsData>(
...
class AnalyticsStreamManager extends AsyncNotifier<AnalyticsData> {
  ...
  Future<AnalyticsData> build() async {
    ...
    final service = AnalyticsService(
        serviceManager.apiService, serviceManager.eventQueueService);
    ...
    final aggregatedMissions = AnalyticsAggregationService.aggregateMissions(
        missions,
        AnalyticsAggregationService.parseTimeframe(filters.timeframe),
        timeline,
        filters.userType);
    final aggregatedEngagements =
        AnalyticsAggregationService.aggregateEngagements(engagements, timeline);
    final aggregatedRetentions =
        AnalyticsAggregationService.aggregateRetention(retentions, timeline);
