...
Map<String, dynamic> normalizeQuestionStats(Map<String, dynamic> raw) {
  return {
    ...raw,
    'questionCount':
        _asInt(raw['questionCount'] ?? raw['totalQuestions'] ?? raw['total']),
    'categoryCount': _asInt(
        raw['categoryCount'] ?? raw['categories'] ?? raw['totalCategories']),
...
Map<String, dynamic> normalizeDatasetInfo(Map<String, dynamic> raw) {
  return {
    ...
    'version':
        (raw['version'] ?? raw['datasetVersion'] ?? 'unknown').toString(),
    'questionCount':
        _asInt(raw['questionCount'] ?? raw['totalQuestions'] ?? raw['total']),
    'source': (raw['source'] ?? 'backend').toString(),
  };
...
Map<String, dynamic> normalizeCategoryStats(
    Map<String, dynamic> raw, QuizCategory category) {
  return {
    ...raw,
...
Map<String, dynamic> normalizeClassStats(Map<String, dynamic> raw) {
  ...
  return {
    ...
    'questionCount':
        _asInt(raw['questionCount'] ?? raw['totalQuestions'] ?? raw['total']),
    'subjectCount': _asInt(raw['subjectCount'] ??
        raw['categoryCount'] ??
