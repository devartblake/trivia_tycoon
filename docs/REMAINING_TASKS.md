...
| Phase 3 - Test coverage (remaining gaps) | Medium | ~4.1% → 40% target; ArcadeGameShell, CryptoWalletScreen, secure channel, leaderboard widgets, LoginManager, AuthTokenStore, AuthStateNotifier, LeaderboardController, QuestionController, UserWallet, CoinBalanceNotifier, EnergyNotifier, WalletService, currentUserIdProvider tests added 2026-05-12 | No |
...
- Implemented backend pattern:
1. Presigned/upload-ticket flow
   ...
   - Response body:
     ...
     ```
   - Frontend uploads bytes directly to `uploadUrl`, then persists the final
     profile value via `PATCH /users/me` using `avatarUrl: publicUrl`.
