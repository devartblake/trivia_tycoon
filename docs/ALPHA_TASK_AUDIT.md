...
- Completed: planning doc created, priority breakdown finalized, smoke script dry-run validated, admin dashboard role-gate hardening implemented, known-good smoke response checklist documented, and contract/error mapping pass implemented for Admin Users + Question Bank.
- Remaining next: propagate P0.2 mapping pattern to remaining admin list screens and run non-dry-run smoke checks against live backend env.
