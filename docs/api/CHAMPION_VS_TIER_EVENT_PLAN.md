...
| **2 — Live rounds (done)** | ✅ Backend: `ChampionRound`/`ChampionRoundAnswer` entities, `ChampionMatchOrchestrator` (start → broadcast → answer window → resolve → next/close), Hangfire-scheduled resolves, REST answer endpoint, SignalR round contracts, scheduler Start hook. ✅ Client: NotificationHub round streams + `joinGameEvent`, `submitRoundAnswer`, `ChampionLiveScreen` (question + countdown + answer + resolve/ended feedback), card "Watch the battle live". Reconnect/replay-on-join polish is the remaining refinement. | ~3–4 days | Phase 1 + NotificationHub |
